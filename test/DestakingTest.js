...
const DestakingJson = require('../build/contracts/DestakingFarmTest.json');
const domJson = require('../build/contracts/DominationToken.json');
...
beforeEach(async () => {
    ...
    dom = await new web3.eth.Contract(domJson['abi'])
        .deploy({ data: domJson['bytecode'] })
        .send({ from: owner, gas: GAS });
    Destaking = await new web3.eth.Contract(DestakingJson['abi'])
        .deploy({ data: DestakingJson['bytecode'], arguments: ["Test Staking", dom._address, STAKING_CAP] })
        .send({ from: owner, gas: '5000000' });
    contractAddress = Destaking._address;
    // Approve the owner
    await dom.methods.approve(contractAddress, STAKING_CAP).send({from: owner});
    const allowance = await dom.methods.allowance(owner, contractAddress).call();
    abiDecoder.addABI(DestakingJson['abi']);
    console.log('Owner allowance is', allowance.toString());
});

async function allow(addr, amount) {
    await dom.methods.transfer(addr, amount).send({from: owner, gas: GAS});
    await dom.methods.approve(contractAddress, amount).send({from: addr});
    const allowance = await dom.methods.allowance(addr, contractAddress).call();
    assert(allowance.toString() === amount.toString(), 'Allowance didn\'nt happen');
}

async function addReward() {
    return Destaking.methods.addReward(1000, 500).send({from: owner, gas: GAS});
}

async function call(method, ...args) {
    return await Destaking.methods[method](...args).call();
}
...
async function balance(addr) {
    const res = await dom.methods.balanceOf(addr).call();
    return res.toString();
}
...
async function setUpStakes() {
    await addReward();
    await allow(ac1, 200);
    const tx = await Destaking.methods.stake(100).send({from: ac1, gas: GAS});
    await getTransactionLogs(tx.transactionHash);
    let stake = await Destaking.methods.stakeOf(ac1).call();

    await Destaking.methods.stake(100).send({from: ac1, gas: GAS});
    stake = await Destaking.methods.stakeOf(ac1).call();
    console.log('ac1 staked ', stake);

    await allow(ac2, 1000);
    await Destaking.methods.stake(1000).send({from: ac2, gas: GAS});
    stake = await Destaking.methods.stakeOf(ac2).call();
    const allowance = await dom.methods.allowance(ac2, contractAddress).call();
...
describe('Happy Destaking', () => {
    it('Sets the reward', async () => {
        const totalRewBefore = await Destaking.methods.totalReward().call();
        assert.deepStrictEqual(totalRewBefore, '0');
        await Destaking.methods.addReward(100, 10).send({from: owner, gas: GAS});
        let totalRewAfter = await Destaking.methods.totalReward().call();
        assert.deepStrictEqual(totalRewAfter, '100');
        let earlyWithdrawReward = await Destaking.methods.earlyWithdrawReward().call();
        assert.deepStrictEqual(earlyWithdrawReward, '10');

        await Destaking.methods.addReward(50, 40).send({from: owner, gas: GAS});
        totalRewAfter = await Destaking.methods.totalReward().call();
        assert.deepStrictEqual(totalRewAfter, '150');
        earlyWithdrawReward = await Destaking.methods.earlyWithdrawReward().call();
    ...
    it('Withdraw right after it opens gives no reward', async function() {
        ...
        await Destaking.methods.setEarlyWithdrawalPeriod(0).send({from: owner, gas: GAS});
        ...
        const tx = await Destaking.methods.withdraw(400).send({from: ac2, gas: GAS});
        await getTransactionLogs(tx.transactionHash);
    ...
    it('Withdraw halfway before it ends', async function () {
        ...
        await Destaking.methods.setEarlyWithdrawalPeriod(30000).send({from: owner, gas: GAS});
        ...
        await Destaking.methods.withdraw(400).send({from: ac2, gas: GAS});
        let after = await vars();
        let bal = await balance(ac2);
        assert.deepStrictEqual(after, {
            stakedTotal: '1000',
    ...
    it('Withdraw right before close', async function() {
        ...
        await Destaking.methods.setEarlyWithdrawalPeriod(59990).send({from: owner, gas: GAS});
        ...
        await Destaking.methods.setEarlyWithdrawalPeriod(60000).send({from: owner, gas: GAS});
        ...
        let stakes = await Destaking.methods.stakeOf(ac2).call();
        assert.deepStrictEqual(stakes, '0');

        // Withdraw ac1
        await Destaking.methods.withdraw(200).send({from: ac1, gas: GAS});
        bal = await balance(ac1);
        assert.deepStrictEqual(bal, (801 * 200 / 600 + 200).toString());
        stakes = await Destaking.methods.stakeOf(ac1).call();
    ...
    it('annual rate calculation', async function() {
        ...
        await Destaking.methods.setEarlyWithdrawalPeriod(withdrawTime).send({from: owner, gas: GAS});
