...
contract Destaking {
    ...
    constructor (string memory name_,
        ...
        uint256 stakingCap_) {
        name = name_;
        require(tokenAddress_ != address(0), "Destaking: 0 address");
        tokenAddress = tokenAddress_;

        require(stakingStarts_ > 0, "Destaking: zero staking start time");
        ...
        require(stakingEnds_ > stakingStarts, "Destaking: staking end must be after staking starts");
        stakingEnds = stakingEnds_;

        require(withdrawStarts_ >= stakingEnds, "Destaking: withdrawStarts must be after staking ends");
        withdrawStarts = withdrawStarts_;

        require(withdrawEnds_ > withdrawStarts, "Destaking: withdrawEnds must be after withdraw starts");
        withdrawEnds = withdrawEnds_;

        require(stakingCap_ > 0, "Destaking: stakingCap must be positive");
    ...
    returns (bool) {
        require(rewardAmount > 0, "Destaking: reward must be positive");
        require(withdrawableAmount >= 0, "Destaking: withdrawable amount cannot be negative");
        require(withdrawableAmount <= rewardAmount, "Destaking: withdrawable amount must be less than or equal to the reward amount");
    ...
    */
    ...
    returns (bool) {
        address from = msg.sender;
        require(amount <= _stakes[from], "Destaking: not enough balance");
    ...
    returns (bool) {
        ...
        require(remaining > 0, "Destaking: Staking cap is filled");
        require((remaining + stakedTotal) <= stakingCap, "Destaking: this will increase staking amount pass the cap");
    ...
    modifier _realAddress(address addr) {
        require(addr != address(0), "Destaking: zero address");
        _;
    }

    modifier _positive(uint256 amount) {
        require(amount >= 0, "Destaking: negative amount");
        _;
    }

    modifier _after(uint eventTime) {
        require(block.timestamp >= eventTime, "Destaking: bad timing for the request");
        _;
    }
    ...
    modifier _hasAllowance(address allower, uint256 amount) {
        ...
        require(amount <= ourAllowance, "Destaking: Make sure to add enough allowance");
        _;
    }
}

// File: contracts/DestakingFarmTest.sol


pragma solidity 0.8.4;


contract DestakingFarmTest is Destaking {
    ...
    constructor (string memory name_,
        ...
        uint256 stakingCap_)
    Destaking(name_, tokenAddress_, block.timestamp, block.timestamp + GAP, block.timestamp + GAP, block.timestamp + GAP * 2, stakingCap_) { }
